...
public interface RecipeRepository extends JpaRepository<Recipe, Integer> {
    ...
    @Query("SELECT DISTINCT r FROM Recipe r "
            ...
            + "AND r.status = com.david.NUTRITION_TRACNKER.entity.enums.RecipeStatus.ACTIVE")
    List<Recipe> findRecipesByIngredientIds(@Param("ids") List<Integer> ids);

    // 2. Tìm tất cả Active
    @Query("SELECT r FROM Recipe r WHERE (r.isDeleted IS NULL OR r.isDeleted = false) AND r.status = com.david.NUTRITION_TRACNKER.entity.enums.RecipeStatus.ACTIVE")
    List<Recipe> findAllActiveRecipes();

    List<Recipe> findByStatus(RecipeStatus status, Pageable pageable);

    // 3. Tìm theo tên
    @Query("SELECT r FROM Recipe r WHERE r.name LIKE %:keyword% AND (r.isDeleted IS NULL OR r.isDeleted = false) AND r.status = com.david.NUTRITION_TRACNKER.entity.enums.RecipeStatus.ACTIVE")
    ...
    @Query(value = "SELECT * FROM Recipes ORDER BY RAND() LIMIT 50", nativeQuery = true)
